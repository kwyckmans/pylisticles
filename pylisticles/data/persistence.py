...
class CollectionStorage:
    ...
    def _collection_to_yaml_data(self, collection: Collection) -> Dict[str, Union[Dict[str, str], List[Dict[str, Union[str, bool, List[str]]]]]]:
    ...
    def _markdown_table_to_items(self, markdown_content: str, fields: List[Field]) -> List[Item]:
    ...
    def save_collection(self, collection: Collection) -> None:
        ...
        try:
            ...
            raise PersistenceError(f"Failed to save collection '{collection.name}': {e}") from e
    ...
    def load_collection(self, collection_name: str) -> Collection:
        ...
        try:
            ...
            if not content.startswith('---\n'):
                raise PersistenceError(f"Invalid file format for '{collection_name}': missing YAML frontmatter")
            
            parts = content[4:].split('\n---\n', 1)
            if len(parts) != 2:
                raise PersistenceError(f"Invalid file format for '{collection_name}': malformed frontmatter")
            ...
            raise PersistenceError(f"Failed to load collection '{collection_name}': {e}") from e
    ...
    def delete_collection(self, collection_name: str) -> None:
        ...
        try:
            ...
            raise PersistenceError(f"Failed to delete collection '{collection_name}': {e}") from e
